...
print(f"特征: {X.shape[1]}维(15+{ind.shape[1]}行业) 测试集: {MASK.sum()}条")
...
for ty in TY:
    tr = (yrs != ty); te = (yrs == ty)
    train_save(X[tr].values, y[tr], os.path.join(MDIR, f"FULL_IND_{ty}.pt"))
    print(f"  fold {ty}: 训{tr.sum()} 测{te.sum()}")

pr = np.zeros(len(u), dtype=int)
for ty in TY:
    te = (yrs == ty); pr[te] = load_pred(X[te].values, os.path.join(MDIR, f"FULL_IND_{ty}.pt"))

cm = confusion_matrix(y[MASK], pr[MASK]); n = cm.sum(); acc = sum(cm[i,i] for i in range(5))/n*100
print(f"\n=== FULL_IND→S1234 ({n}条 acc={acc:.1f}%) ===")
...
print(f"行业分布:")
for ind in sorted(u.loc[buy,"industry"].unique()):
    n = sum(buy & (u.industry == ind)); r = rets[buy & (u.industry == ind)]
    print(f"  {ind}: {n}只 均{r.mean()*100:.1f}%")
print(f"\n对比: FULL(无行业) 194只年+22.4% / NoInd(4行业) 206只年+23.7%")
