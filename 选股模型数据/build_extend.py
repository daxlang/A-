...
# 从缓存自动检测可用季度
QS = sorted(set(fn[5:13] for fn in os.listdir(CACHE) if fn.startswith("yjbb_") and fn.endswith(".parquet")))
...
data["quarter"] = (data["qdate"].str[4:6].astype(int) - 1) // 3 + 1
...
for i, code in enumerate(todo):
    ...
    try:
        df_p = ak.stock_zh_a_hist(symbol=code, period="daily", start_date="20190101", end_date="20260630", adjust="qfq")
...
res.to_csv(os.path.join(OUT, "training_extended.csv"), index=False)
print(f"\n保存: training_extended.csv {len(res)}条 usable={res.usable.sum()}")
