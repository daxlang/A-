"""架构扫描(轻量版: 1000 epoch, 8B batch拆小)"""
...
archs = {
    "128x64_d02": lambda: nn.Sequential(nn.Linear(20,128),nn.ReLU(),nn.Dropout(0.2),nn.Linear(128,64),nn.ReLU(),nn.Dropout(0.2),nn.Linear(64,5)),
    "256x128_d02": lambda: nn.Sequential(nn.Linear(20,256),nn.ReLU(),nn.Dropout(0.2),nn.Linear(256,128),nn.ReLU(),nn.Dropout(0.2),nn.Linear(128,5)),
    "128x128x64_d02": lambda: nn.Sequential(nn.Linear(20,128),nn.ReLU(),nn.Dropout(0.2),nn.Linear(128,128),nn.ReLU(),nn.Dropout(0.2),nn.Linear(128,64),nn.ReLU(),nn.Dropout(0.2),nn.Linear(64,5)),
    "128x64_d04": lambda: nn.Sequential(nn.Linear(20,128),nn.ReLU(),nn.Dropout(0.4),nn.Linear(128,64),nn.ReLU(),nn.Dropout(0.4),nn.Linear(64,5)),
}

results = []
for name, mk_net in archs.items():
    net = mk_net()
    n_params = sum(p.numel() for p in net.parameters())
    sc = StandardScaler(); Xs = torch.FloatTensor(sc.fit_transform(X_tr)); ys = torch.LongTensor(y_tr)
    ...
    with torch.no_grad(): pr = torch.argmax(net(Xte), dim=1).numpy()
    ...
    yr = np.mean(yrly)
    print(f"{name:<20s} 参{n_params:>5d}  acc={acc:>5.1f}%  买{nb:>3d}只  年{yr:>+5.1f}%  召回{cm[4,4]}/{cm[4].sum()}")
    results.append((name, n_params, acc, nb, yr, cm[4,4]/cm[4].sum()*100))
    torch.save({"model":bs,"scaler":sc}, os.path.join(MDIR, f"ARCH_{name}.pt"))
