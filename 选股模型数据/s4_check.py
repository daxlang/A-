...
LAB = ["暴跌","跌","小涨","中涨","暴涨"]
S13 = (u.cfo_to_revenue >= 0) & (u.current_ratio >= 0.5) & (u.gm_pct >= 0.1)
S134 = S13 & (u.liability_to_asset <= 0.8)
...
for name, m in [("S1∩S3(基准)", S13), ("S1∩S3∩S4", S134)]:
    cm = confusion_matrix(y[m], pr[m]); n = cm.sum(); acc = sum(cm[i,i] for i in range(5))/n*100
