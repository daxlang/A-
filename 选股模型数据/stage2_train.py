...
# 阶段2排除: cfo_to_profit < 0 且 net_margin > 0
S2_EXCLUDE = (u.cfo_to_profit < 0) & (u.net_margin > 0)
print(f"阶段1: S1={S1.sum()}/{len(u)}  阶段2排除: {S2_EXCLUDE.sum()}只")
print(f"合并后(S1 & !S2): {(S1 & ~S2_EXCLUDE).sum()}")
...
def train_save(Xtr, ytr, path):
    sc = StandardScaler()
    Xs = torch.FloatTensor(sc.fit_transform(Xtr)); ys = torch.LongTensor(ytr)
    ...
    for _ in range(5000):
        ...
        loss = lfn(m(Xs), ys); loss.backward(); opt.step()
...
print("\n训练 S2...")
for ty in TY:
    tr = (yrs != ty) & MASK; te = (yrs == ty) & MASK
    train_save(X[tr].values, y[tr], os.path.join(MDIR, f"S2_{ty}.pt"))
...
for ty in TY:
    te = (yrs == ty) & MASK
    if te.sum() > 0:
        pr_s2[te] = load_predict(X[te].values, os.path.join(MDIR, f"S2_{ty}.pt"))

# 基准: MLP0_5 + 阶段1预筛
pr_old = np.zeros(len(u), dtype=int)
for ty in TY:
    te = (yrs == ty)
    pr_old[te] = load_predict(X[te].values, os.path.join(MDIR, f"MLP0_5_{ty}.pt"))

# 对比
for name, pr, mask in [
    ("MLP0_5+S1预筛", pr_old, S1),
    ("MLP0_5+S1+S2", pr_s2, MASK),
]:
